...
<x-app-layout>
    ...
    @if (auth()->user()->role == 'Client')
        <x-clients-task-view :data="$data" :request="$request" />
    @else
        @php
            if (!$request) {
                $request = [
            ...
            <form action="{{ route('tasks.index') }}" method="GET">
                <div class=" flex justify-start items-end gap-x-5">
                    <label class="form-control w-full max-w-xs">
                        ...
                        <select
                            class="select select-bordered bg-transparent border text-[1rem] border-gray-400 text-black"
                            name="AssignedTo">
                            <option class="text-[1rem]" value="Me"
                                {{ old('AssignedTo', $request['AssignedTo']) == 'Me' ? 'selected' : '' }}>لي / انا
                            </option>
                            ...
                            <option class="text-[1rem]" value="Others"
                                {{ old('AssignedTo', $request['AssignedTo']) == 'Others' ? 'selected' : '' }}>اخرى
                            </option>
                        </select>
                    </label>
                    ...
                    <label class="form-control w-full max-w-xs">
                        ...
                        <select
                            ...
                            <option class="text-[1rem]" value="Incomplete"
                                {{ old('TaskStatus', $request['TaskStatus']) == 'Incomplete' ? 'selected' : '' }}>غير
                                مكتملة
                            </option>
                    ...
                    <label class="form-control w-full max-w-xs">
                        ...
                        <select
                            ...
                            <option class="text-[1rem]" value="LastYear"
                                {{ old('duration', $request['duration']) == 'LastYear' ? 'selected' : '' }}>العام
                                الحالي
                            </option>
                            <option class="text-[1rem]" value="date"
                                {{ old('duration', $request['duration']) == 'date' ? 'selected' : '' }}>تاريخ معين
                            </option>
                        </select>
                    </label>
                ...
                <div class=" justify-center gap-3 {{ old('duration', $request['duration']) == 'date' ? 'flex' : 'hidden' }}"
                    ...
                    <label class="form-control">
                        ...
                        <input type="date" class="rounded-lg bg-transparent text-black border border-gray-400"
                            name="data_from" id=""
                            value="{{ old('data_from', $request['data_from'] ?? '') }}">
                    </label>
                    <label class="form-control">
                        <div class="label">
                    ...
                    <tbody class="text-start" id="table_body">
                        ...
                        @foreach ($data['tasks'] as $task)
                            <tr class=" border-[#E6E8EB]">
                                ...
                                <td>{{ $task->title }}</td>
                                <td><span
                                        class="{{ $task->priority['class'] }} ">{{ __($task->priority['name']) }}</span>
                                </td>
                                <td>{{ $task->due_date }}</td>
                                ...
                                <td>
                                    <div class="flex gap-x-3">
                                        ...
                                        <div class="tooltip" data-tip="اضغط لحذف المهمة المهمة">
                                            <button><i class="text-lg fa-regular fa-trash-can"></i></button>
                                        </div>
                                        {{-- <button><i class="text-lg fa-solid fa-pen"></i></button>
                                        <button><i class="text-lg fa-regular fa-bell"></i></button> --}}
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                ...
                <form action="{{ route('tasks.store') }}" method="POST">
                    ...
                    <div class="grid grid-flow-row gap-y-5">
                        @if (auth()->user()->role == 'Lawyer')
                            ...
                            <div class="grid grid-flow-col gap-x-1 items-center">
                                ...
                                <div class="col-span-8">
                                    <select id="client_case_id" name="client_case_id"
                                        value="{{ old('client_case_id') }}"
                        ...
                        @elseif(auth()->user()->role == 'Manager')
                            <input type="hidden" name="TaskforMe" value="ON">
                        @endif
                        ...
                        <div class="grid grid-flow-col gap-x-1 items-center">
                            <div class="col-span-4 flex flex-col items-center">
                                <label for="due_date"
                                    class="text-sm font-medium text-gray-700">{{ __('لغاية تاريخ') }}
                                    <span class="text-red-500">*</span>
                                </label>
                                @error('due_date')
                                    <p class="text-sm text-red-500 text-start">
                                ...
                                <select id="assignTo" name="assignTo"
                                    ...
                                    @php
                                        if (auth()->user()->role == 'Lawyer') {
                                            ...
                                            $officeLawyers = auth()
                                                ->user()
                                                ->office->users->where('role', 'Lawyer')
                                                ->where('id', '!=', auth()->id());
                                        }
                                                ...
                                                @foreach ($officeClients as $client)
                                                    @if ($client->user_id)
                                                        <option value="{{ $client->user_id }}"
                                                            ...
                                                            {{ $client->full_name }}</option>
                                                    @else
                                                        <option disabled>{{ $client->full_name }} - (لم يسجّل بعد)
                                                        </option>
                                                    @endif
                                                @endforeach
                                            @else
                        ...
                        <div class="grid grid-flow-col gap-x-1 items-center">
                            <div class="col-span-4 flex flex-col items-center">
                                <label for="description"
                                    class="text-sm font-medium text-gray-700">{{ __('الوصف') }}
                                    <span class="text-red-500">*</span>
                                </label>
                                @error('description')
                                    <p class="text-sm text-red-500 text-start">
                        ...
                        <div class="form-control">
                            <label class="label cursor-pointer mx-auto flex items-center">
                                ...
                                <span
                                    class="label-text ms-2  text-gray-700">{{ __('تلقي تنبيه بشأن هذه المهمة') }}</span>
                            </label>
                        </div>
                        @error('Reminder')
                            <p class="text-sm text-red-500 text-center">
