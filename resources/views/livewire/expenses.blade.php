                ...
                <tbody>
                    ...
                    @foreach ($data['expenses'] as $expense)
                        <tr
                            ...
                            <td>{{ number_format($expense->quantity, 0) }}</td>
                            ...
                            @endphp
                            <td><span
                                    class="text-xs {{ $class }} font-bold rounded-2xl text-white px-3 py-1">{{ $isPaid ? 'مدفوعة' : 'غير مدفوعة' }}</span>
                            </td>
                            </td>
                            <td>
                                <a href="{{ route('clients.show', $legalCase->client->id) }}"
                                    class="hover:underline">{{ $legalCase->client->full_name }}</a>
                            </td>
                            <td><a href="{{ route('legalCases.show', $legalCase->id) }}"
                                    class="hover:underline">{{ $legalCase->title }}</a></td>
                            <td>
                                <div class=" flex justify-center items-center h-full gap-x-2">
                                    <div>
                                        <button><i class="text-[1rem] fa-solid fa-pen-to-square"></i></button>
                                    </div>
                                    <div>
    ...
    <dialog id="addExpenss" class="modal modal-middle sm:modal-middle" style="width:90%;">
        <div class="modal-box text-black bg-white text-lg" style="width: 90%;">
            ...
            @php
                ...
                $excludedActivities = ['قراءة ملف', 'زيارة طرف اخر'];
                $activities = \app\Models\expense::pluck('activity')
                    ->unique()
                    ->whereNotIn('activity', $excludedActivities);
            @endphp
                ...
                <form action="{{ route('expenses.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <div class="flex items-center">
                            ...
                            <select id="case" name="case"
                                ...
                                <option value="" selected disabled>{{ __('Select a Legal Case') }}</option>
                    ...
                    <div class="flex items-center mb-2">
                        ...
                        <select id="form_select_activity" name="form_select_activity"
                            ...
                            <option value="قراءة ملف"
                                {{ old('form_select_activity') == 'قراءة ملف' ? 'selected' : '' }}>قراءة ملف</option>
                            <option value="زيارة طرف اخر"
                                {{ old('form_select_activity') == 'زيارة طرف اخر' ? 'selected' : '' }}>زيارة طرف اخر
                            </option>
                            @foreach ($activities as $activity)
                                @if ($activity != 'قراءة ملف' && $activity != 'زيارة طرف اخر')
                                    <option value="{{ $activity }}"
                                        {{ old('form_select_activity') == $activity ? 'selected' : '' }}>
                                        {{ $activity }}</option>
                                @endif
                            @endforeach
                        </select>
                        <div class="justify-end ms-4">
                            <button type="button" id="add_new_activity"
                        ...
                        <div class="flex flex-wrap  justify-center gap-6">
                            ...
                            <div class="relative">
                                ...
                                <div class="relative mt-5">
                                    ...
                                    <input type="text" id="form_cost" name="form_cost"
                                        value="{{ old('form_cost') }}"
                                        class="ps-10 p-2 w-full border lg:text-[85%] rounded-md border-[#cacaca] focus:border-[#E1E1E1] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300">
                                </div>
                                @error('form_cost')
                                    <p class="text-sm text-red-500">
                                        * {{ __($message) }}
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="form_quantity" value="{{ old('form_quantity') }}"
                                    name="form_quantity"
                                    class="mt-5 w-full p-2 border border-[#cacaca] rounded-md focus:border-[#E1E1E1] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300">
                            </div>
                            @error('form_quantity')
                                <p class="text-sm text-red-500">
                                    * {{ __($message) }}
            ...
            @else
                <div class="flex justify-center items-center w-full mt-3">
                    <h1>لا يوجد قضايا لإضافة نفقات لها</h1>
                </div>
            @endif
        </div>
    </dialog>
...
<script>
    ...
    addActivityButton.addEventListener('click', function() {
        ...
        newActivityInput.name = 'new_activity';
        newActivityInput.placeholder = 'أدخل اسم النشاط الجديد'; // Placeholder text for new activity
        newActivityInput.classList =
            'border border-black bg-transparent p-1 text-black rounded-md focus:border-[#E1E1E1] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300';
